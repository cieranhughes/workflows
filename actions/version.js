...
(async () => {
  ...
  const commitMessage = process.env['INPUT_COMMIT-MESSAGE'] || 'version bump to {{version}}';
  ...
  try {
    ...
    // set git user
    await runInWorkspace('git', ['config', 'user.name', `"${process.env.GITHUB_USER || 'Eithan.AI'}"`]);
